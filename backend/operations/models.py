    ...
    def validate_and_complete(self):
        ...
        for item in self.items.all():
            ...
            try:
                ...
                return False, f"No stock available for {item.product.name} in {self.warehouse.name}"
            
            if stock_item.available_quantity() < requested_quantity:
                return False, f"Insufficient stock for {item.product.name}. Available: {stock_item.available_quantity()}, Required: {requested_quantity}"
    ...
    def validate_and_complete(self):
        ...
        for item in self.items.all():
            ...
            try:
                ...
                return False, f"No stock available for {item.product.name} in source warehouse {self.warehouse.name}"
            
            if from_stock.available_quantity() < transfer_qty:
                return False, f"Insufficient stock for {item.product.name} in source warehouse. Available: {from_stock.available_quantity()}, Required: {transfer_qty}"
